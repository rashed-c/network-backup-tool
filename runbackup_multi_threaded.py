...
def get_credential():
    ...
    '''
    #username = [{'uname': "usernamehere"}]
    #password = [{'pwd':"switchpw_here"}]
    #fgt_pwd = [{'fgt_pwd':"fw_pw_here"}]
...
def get_devices(creds,config_file):
    ...
    try:
            ...
            for device_category in cfg['DeviceList'].keys():
                if device_category == "Switch_List":
                    ...
                    for device in cfg['DeviceList'][device_category]:
                        cfg['DeviceList'][device_category][device]['username'] = creds[0][0]['uname'] # Add username to to device dictionary
                        cfg['DeviceList'][device_category][device]['password'] = creds[1][0]['pwd']
                        cfg['DeviceList'][device_category][device]['secret'] = "secret"
                        device_list.append(cfg['DeviceList'][device_category][device])
                                
                elif device_category == "Firewall_List":
                    log.info("Devices detected in: {0}".format(device_category))
                    for device in cfg['DeviceList'][device_category]:
                        cfg['DeviceList'][device_category][device]['username'] = creds[0][0]['uname'] # Add username to to device dictionary
                        cfg['DeviceList'][device_category][device]['password'] = creds[2][0]['fgt_pwd']  # Add fortigate password to dictionary
                        cfg['DeviceList'][device_category][device]['username'] = creds[0][0]['uname'] # Add username to to device dictionary
                        cfg['DeviceList'][device_category][device]['password'] = creds[2][0]['fgt_pwd']  # Add fortigate password to dictionary
                        device_list.append(cfg['DeviceList'][device_category][device])
        else:
        ...
        log.info("Error?? - Skipping backup for site: {0}".format(cfg['site']))
        pass
    if device_list: 
...
#Run backups                               
def run_backup(configs):
    log = logging.getLogger("run_backup()")
    config_file = configs
    creds = get_credential()
    today= date.today()  #hold todays date
    path=os.environ['userprofile']
    all_devices = get_devices(creds,read_configfile(config_file))
    if all_devices:
        site = all_devices[1]
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        executor.map(run_backup, config_file)
